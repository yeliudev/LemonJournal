...
module.exports = async ctx => {
    ...
    if (open_id_object) {
        ...
        try {
            ...
            ctx.body = {
                success: true,
                data: await mysql('journal').where({ journal_book_id: ctx.query.journal_book_id }).select('journal_id', 'title', 'previewUrl', 'last_update_time').orderBy('last_update_time', 'desc')
            }
        } catch (error) {
            ctx.body = {
                success: false,
